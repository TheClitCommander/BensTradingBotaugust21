...
// Configuration
const ENABLE_STUBS = process.env.ENABLE_STUBS === 'true';
...
// Helper function to safely read numeric query parameters
function readNumeric(q, defVal) {
  if (q == null) return defVal;
  if (typeof q === 'object') {
    const cand = q.limit ?? q.value ?? Object.values(q)[0];
...
// Proxy routes to FastAPI (add these BEFORE any catch-alls that might shadow)
app.use('/api/risk',       createProxyMiddleware({ target: PY_API, changeOrigin: true }));
app.use('/api/decisions',  createProxyMiddleware({ target: PY_API, changeOrigin: true }));
...
app.use('/api/opportunities', createProxyMiddleware({ target: PY_API, changeOrigin: true }));
...
// Helper function for open orders
function isOpenStatus(s) {
  return s === 'accepted' || s === 'replaced' || s === 'pending_cancel';
}

// Open orders endpoint (add BEFORE the :id parameter route)
app.get('/api/paper/orders/open', (req, res) => {
  res.json(paperOrders.filter(o => isOpenStatus(o.status)));
});

// Friendly aliases so UI fallbacks all work
app.get('/api/orders/open', (req, res) => res.redirect(307, '/api/paper/orders/open'));
app.get('/api/paper/open-orders', (req, res) => res.redirect(307, '/api/paper/orders/open'));
...
app.get('/api/quotes', (req, res) => {
  ...
  if (!ENABLE_STUBS) {
    return res.status(502).json({ error: 'quotes upstream not configured' });
  }
  ...
  res.json({ proxied: false, quotes: items });
});

app.get('/api/bars', (req, res) => {
  const symbol = String(req.query.symbol || req.query.symbols || 'AAPL');
  const timeframe = String(req.query.timeframe || '1Day');
  const limit = Math.min(readNumeric(req.query.limit, 30), 500);
  
  if (!ENABLE_STUBS) {
    return res.status(502).json({ error: 'bars upstream not configured' });
  }
...
// --- OPPORTUNITIES: candidates with detailed narrative ---
app.get('/api/opportunities', (req, res) => {
  if (!ENABLE_STUBS) {
    return res.status(502).json({ error: 'opportunities upstream not configured' });
  }
  
  // Generate mock opportunities with detailed narrative fields
  const opportunities = [
    {
      id: '1',
      symbol: 'SPY',
